...
import {
  ...
  Target, X, Upload, Sparkles, ChevronRight, Filter, LayoutGrid,
...
const DEFAULT_SPEC_KEYS = ['處理器', '掃描引擎', '電池容量', '防護等級', '作業系統', '重量'];
...
export default function Dashboard() {
  ...
  const [hiddenSpecKeys] = useState<Set<string>>(new Set());
  const [expandedChart, setExpandedChart] = useState<'radar' | 'scatter' | null>(null);
  const [radarMetrics] = useState(['耐用性', '性能', '續航', '便攜性', '掃描力']);
  ...
  const columnItems = useMemo(
    () => [{ id: 'spec-header', isHeader: true, name: '產品維度', brand: '', image: '', specs: {} as Record<string,string> }, ...competitors],
    [competitors]
  );
    ...
    specKeys.forEach(key => {
      ...
      competitors.forEach(comp => {
        ...
        if (val) {
          ...
          if (match && match[2]) { const u = match[2].trim(); if (u) unitCounts[u] = (unitCounts[u] || 0) + 1; }
        }
      });
      let max = 0, best = '';
  ...
  const visibleCompetitors = useMemo(() => competitors, [competitors]);
  ...
  const openAddModal = () => {
    ...
    if (lastSavedForm) {
      ...
      const defaultKeys = specKeys.length > 0 ? specKeys : DEFAULT_SPEC_KEYS;
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <button onClick={fetchCompetitors} className="px-6 py-2 bg-cyan-500 text-white rounded-xl font-bold">重新載入</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div ref={dashboardRef} className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-cyan-100 selection:text-cyan-900">
      ...
      <main className="max-w-[1600px] mx-auto px-6 py-8 space-y-8">
        ...
        <section className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
          <div className="space-y-4 flex-1">
            <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 sm:text-4xl">工業設計競品分析儀表板</h1>
            <p className="text-slate-500 max-w-2xl">即時追蹤全球工業手持設備、RFID 與掃描器規格，透過 AI 洞察市場真空區，驅動下一代產品設計策略。</p>
            <div className="flex flex-wrap items-center gap-3">
              <div className="relative flex-1 min-w-[300px]">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" />
                <input type="text" placeholder="搜尋型號、品牌或規格關鍵字..."
                    ...
                    <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
                      ...
                      {allBrands.map(brand => (
                        <label key={brand} className="flex items-center gap-2 py-1.5 cursor-pointer group">
                          <input type="checkbox" checked={selectedBrands.has(brand)} className="accent-cyan-500"
                            onChange={() => { const next = new Set(selectedBrands); next.has(brand) ? next.delete(brand) : next.add(brand); setSelectedBrands(next); }} />
                          <span className="text-sm font-medium text-slate-600 group-hover:text-cyan-600">{brand}</span>
                        </label>
                      ))}
                      {selectedBrands.size > 0 && (
                        <button onClick={() => setSelectedBrands(new Set())} className="mt-3 text-xs text-red-400 font-bold w-full text-left">清除篩選</button>
                      )}
                    </motion.div>
                  )}
                </AnimatePresence>
          ...
          <div className="flex items-center gap-3">
            <button onClick={handleExportCSV} className="flex items-center gap-2 px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-slate-700 hover:bg-slate-50 transition-all font-medium shadow-sm">
              <FileDown className="w-4 h-4 text-green-500" />CSV
            </button>
            <button onClick={handleExportPDF} className="flex items-center gap-2 px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-slate-700 hover:bg-slate-50 transition-all font-medium shadow-sm">
              <FileDown className="w-4 h-4 text-red-500" />PDF 簡報
            </button>
            {isAdmin && (
              <button onClick={openAddModal} className="flex items-center gap-2 px-5 py-2.5 bg-cyan-500 text-white rounded-xl hover:bg-cyan-600 transition-all font-bold shadow-lg shadow-cyan-200">
        ...
        <section className={'bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden transition-all duration-500 ' +
          ...
          <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-white">
            <div className="flex items-center gap-4">
              <h2 className="text-lg font-bold flex items-center gap-2"><LayoutGrid className="w-5 h-5 text-cyan-500" />規格對比矩陣</h2>
          ...
          <div className={'overflow-auto relative custom-scrollbar flex-1 ' + (isExpanded ? 'h-full' : 'max-h-[600px]')}>
            {filteredCompetitors.length === 0 ? (
              <div className="flex items-center justify-center h-40 text-slate-400 font-medium">尚無競品資料，請登入後點選「新增競品」開始</div>
            ) : (
                ...
                <thead>
                  <Reorder.Group axis="x" values={columnItems} onReorder={handleReorderColumns} as="tr" className="bg-slate-50/50">
                    {columnItems.map((item: any) => (
                      <Reorder.Item key={item.id} value={item} as="th" dragListener={isAdmin && !item.isHeader}
                        ...
                        {item.isHeader ? (
                          ...
                          <div className="flex items-center justify-between mb-4">
                            <div className="flex items-center gap-4">
                              ...
                              <img src={item.image} alt={item.name} className="w-12 h-12 rounded-lg object-cover border border-slate-200" referrerPolicy="no-referrer" />
                              <div>
                            ...
                            {isAdmin && (
                              <div className="flex items-center gap-1 opacity-0 group-hover/col:opacity-100 transition-opacity">
                                <button onClick={() => handleEditCompetitor(item)} className="p-1.5 hover:bg-cyan-100 rounded-lg text-cyan-400"><Settings2 className="w-3.5 h-3.5" /></button>
                                <button onClick={() => setShowDeleteConfirm(item.id)} className="p-1.5 hover:bg-red-100 rounded-lg text-red-400"><Trash2 className="w-3.5 h-3.5" /></button>
                              </div>
                            )}
                          </div>
                        )}
                  ...
                  {visibleSpecKeys.map(key => (
                    <Reorder.Item key={key} value={key} as="tr" className="group hover:bg-cyan-50/30 transition-colors">
                      <td className="sticky left-0 z-10 bg-white group-hover:bg-cyan-50/50 border-b border-r border-slate-100 p-4 font-bold text-slate-600">
                        <div className="flex items-center gap-2">{isAdmin && <GripVertical className="w-4 h-4 text-slate-300" />}{key}</div>
                      </td>
        ...
        <section className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          <div className="xl:col-span-3 flex items-center justify-between gap-3 mb-2">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">分析對象: {aiInsights?.targetBrand} {aiInsights?.targetName}</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <select value={selectedProductId} onChange={e => { setSelectedProductId(e.target.value); setAiInsights(null); }}
                className="text-sm border border-slate-200 rounded-xl px-3 py-2 bg-white outline-none">
                {competitors.map(c => <option key={c.id} value={c.id}>{c.brand} {c.name}</option>)}
              </select>
          ...
          <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
            <h3 className="text-lg font-bold mb-6 flex items-center gap-2"><div className="w-2 h-6 bg-cyan-500 rounded-full" />優劣勢分析</h3>
            {isLoadingInsights ? (
              <div className="flex items-center justify-center h-32"><Loader2 className="w-8 h-8 text-cyan-400 animate-spin" /></div>
            ) : (
              <div className="space-y-6">
                <div>
                  <div className="text-xs font-bold text-green-600 uppercase mb-3 flex items-center gap-1"><Zap className="w-3 h-3 fill-green-600" /> 優勢 (Pros)</div>
                ...
                <div>
                  <div className="text-xs font-bold text-red-500 uppercase mb-3 flex items-center gap-1"><Zap className="w-3 h-3 fill-red-500" /> 劣勢 (Cons)</div>
                  <ul className="space-y-4">
          ...
          <div className="bg-slate-900 p-8 rounded-3xl text-white shadow-xl">
            <h3 className="text-lg font-bold mb-6 flex items-center gap-2"><div className="w-2 h-6 bg-red-500 rounded-full" />市場稠密區 (紅海標配)</h3>
            {isLoadingInsights ? (
          ...
          <div className="bg-cyan-500 p-8 rounded-3xl text-white shadow-xl shadow-cyan-200">
            <h3 className="text-lg font-bold mb-6 flex items-center gap-2"><div className="w-2 h-6 bg-white rounded-full" />藍海戰略提案</h3>
            {isLoadingInsights ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <RadarChart cx="50%" cy="50%" outerRadius="80%" data={radarData}>
                  ...
                  {visibleCompetitors.map((comp, i) => (
                    <Radar key={comp.id} name={comp.name} dataKey={comp.name} stroke={COLORS[i % COLORS.length]} fill={COLORS[i % COLORS.length]} fillOpacity={0.3} />
                  ))}
                  <Tooltip /><Legend />
                </RadarChart>
              </ResponsiveContainer>
            ...
            <div className="flex-1 min-h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                  ...
                  <Scatter name="產品分佈" data={visibleCompetitors.map(c => ({ name: c.name, x: extractNumber(c.specs[scatterConfig.x] || '0'), y: extractNumber(c.specs[scatterConfig.y] || '0') }))} fill="#06b6d4" />
                </ScatterChart>
              </ResponsiveContainer>
            </div>
          </div>
      ...
      <AnimatePresence>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative w-full max-w-md bg-white rounded-3xl shadow-2xl p-8">
              ...
              <form onSubmit={handleLogin} className="space-y-4">
                <input type="password" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none" placeholder="請輸入管理員密碼" value={password} onChange={e => setPassword(e.target.value)} />
        ...
        {showDeleteConfirm && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative w-full max-w-md bg-white rounded-3xl shadow-2xl p-8 text-center">
              <div className="w-16 h-16 bg-red-50 rounded-2xl flex items-center justify-center mx-auto mb-6"><Trash2 className="w-8 h-8 text-red-500" /></div>
        ...
        {showAddModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative w-full max-w-4xl bg-white rounded-3xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
                {formError && <div className="p-4 bg-red-50 border border-red-100 rounded-2xl text-red-600 text-sm font-bold">{formError}</div>}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <input type="text" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none" placeholder="品牌 (e.g. Zebra)" value={newComp.brand} onChange={e => setNewComp({ ...newComp, brand: e.target.value })} />
                  <input type="text" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none" placeholder="型號 (e.g. TC9300)" value={newComp.name} onChange={e => setNewComp({ ...newComp, name: e.target.value })} />
                </div>

                <div className="space-y-3">
                  <label className="block text-sm font-bold text-slate-700">產品圖片</label>
                  <div className="flex items-start gap-4">
                    {imagePreview && <img src={imagePreview} alt="preview" className="w-16 h-16 rounded-xl object-cover border border-slate-200 shrink-0" referrerPolicy="no-referrer" />}
                    <div className="flex flex-col gap-2 flex-1">
                      <label className="flex items-center gap-2 px-4 py-2.5 bg-slate-100 border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-200 transition-all w-fit">
                        <Upload className="w-4 h-4 text-slate-500" />
                        <span className="text-sm font-medium text-slate-600">上傳圖片至 Google Drive</span>
                        <input type="file" accept="image/*" className="hidden" onChange={handleImageChange} />
                      </label>
                      <input type="text" className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none text-sm"
                        placeholder="或直接貼上圖片網址 (https://...)"
                        value={newComp.image}
                        onChange={e => { setNewComp({ ...newComp, image: e.target.value }); setImagePreview(e.target.value); }} />
                    </div>
                  </div>
                </div>

                <div className="space-y-3 p-5 bg-slate-50 rounded-2xl border border-slate-200">
                  <label className="block text-sm font-bold text-slate-700 flex items-center gap-2"><Sparkles className="w-4 h-4 text-cyan-500" />AI 規格智慧解析</label>
                  <textarea className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl outline-none text-sm resize-none" rows={4}
                    placeholder="貼上官網或說明書的原始規格文字，AI 將自動解析成結構化欄位..."
                    value={rawSpecText} onChange={e => setRawSpecText(e.target.value)} />
                  <button onClick={handleAiAnalyze} disabled={isAnalyzing || !rawSpecText.trim()}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {formSpecs.map((spec, idx) => (
                      <div key={idx} className="flex items-center gap-2 p-3 bg-slate-50 border border-slate-200 rounded-xl group">
                        <input type="text" className="w-1/4 text-[10px] font-bold text-slate-500 bg-transparent outline-none" placeholder="規格名稱" value={spec.key} onChange={e => handleUpdateSpecField(idx, 'key', e.target.value)} />
                        <input type="text" className="flex-1 bg-transparent text-sm outline-none font-medium" placeholder="數值" value={spec.value} onChange={e => handleUpdateSpecField(idx, 'value', e.target.value)} />
                        <input type="text" className="w-1/4 bg-white border border-slate-200 rounded px-2 py-1 text-[10px] outline-none" placeholder="單位" value={spec.unit} onChange={e => handleUpdateSpecField(idx, 'unit', e.target.value)} />
                        <button onClick={() => handleRemoveField(idx)} className="p-1 text-slate-300 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 className="w-3.5 h-3.5" /></button>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="p-8 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-3 shrink-0">
                ...
                <button onClick={handleSaveCompetitor} disabled={isSaving} className="px-8 py-2.5 bg-slate-900 text-white font-bold rounded-xl shadow-lg flex items-center gap-2 disabled:opacity-50">
      ...
      {expandedChart && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-8 bg-slate-900/70 backdrop-blur-sm" onClick={() => setExpandedChart(null)}>
          <div className="bg-white rounded-3xl w-full max-w-4xl h-[70vh] p-8 flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-bold">{expandedChart === 'radar' ? '綜合能力雷達圖' : '規格散佈圖'}</h3>
              <button onClick={() => setExpandedChart(null)} className="p-2 hover:bg-slate-100 rounded-full"><X className="w-5 h-5" /></button>
            </div>
